...
class Player(object):
    ...
    def get_block(self):
        ...
        if self.play_state == PlayState.PLAYING:
            try:
                ...
                if (
                    self._playback_queue.qsize() < PLAYBACK_QUEUE_MINIMUM_SIZE
                    and self.filling_queue_done()
                ):
                    self._request_filling_queue()
            except Empty:
    ...
    def _fill_queue(self):
        """Fill the queue at least to its minimum size while checking for end
        of playback.
        ...
        while (
            self._playback_queue.qsize() < PLAYBACK_QUEUE_MINIMUM_SIZE
            and not self._everything_queued
        ):
            if (
                self._end_of_playlist_reached()
                and self.loop_state == LoopState.LOOP
            ):
                self._next_file_index = 0
            ...
            if (
                self._end_of_playlist_reached()
                and self.loop_state == LoopState.SINGLE
            ) or not self._filepaths:
                if self._leftover_audio.shape[1] > 0:
                    padded_leftover = np.zeros(
                        (self._leftover_audio.shape[0], self._block_size),
                        dtype=np.float32,
                    )
                    padded_leftover[
    ...
    def _read_and_queue(self, file_index):
        audio, fs = sf.read(str(self._filepaths[file_index]), dtype="float32")
        assert fs == self._fs

        remaining = (
            audio.reshape(1, -1) if audio.ndim == 1 else audio.transpose()
        )

        required_samples_for_leftover = (
            self._block_size - self._leftover_audio.shape[1]
        )
        fill, remaining = np.hsplit(remaining, [required_samples_for_leftover])
        ...
        while remaining.shape[1] >= self._block_size:
            block, remaining = np.hsplit(remaining, [self._block_size])
            self._playback_queue.put(block)
